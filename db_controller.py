...
class DbController:
    ...
    def add_project(self, description, deadline):
        created = datetime.now()
        sql_add_project = (
            "INSERT INTO Projects (Description, Deadline, Created) VALUES (?,?,?)"
        )
    ...
    def delete_project_only(self, project_id):
        self.query(
            "UPDATE Tasks SET ProjectID = NULL WHERE ProjectID = ?", (project_id,)
        )
    ...
    def check_project_tasks_completed(self, project_id):
        sql_check_project = (
            "SELECT TaskID FROM Tasks WHERE ProjectID = ? AND Completed IS NULL"
        )
    ...
    def get_completed_projects(self):
        results = self.select_query(
            "SELECT * FROM Projects WHERE Completed IS NOT NULL"
        )
        return results
