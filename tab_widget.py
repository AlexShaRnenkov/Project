...
from table_widget import TasksTable, ProjectsTable
from add_new_dialog import NewTaskDialog, NewProjectDialog
from mark_complete_dialog import (
    MarkProjectTasksCompleteDialog,
    MarkProjectCompleteDialog,
)
from edit_dialog import EditTaskDialog, EditProjectDialog
from delete_dialog import DeleteTaskDialog, DeleteProjectDialog
from project_tasks_dialog import ProjectTasksDialog


class TaskProjectTabs(QWidget):
    def __init__(self):
        super().__init__()
        self.controller = DbController("to_do.sqlite")
        ...
        self.tasks_radio_buttons = RadioButtonWidget(
            ["Active Tasks", "Completed Tasks", "All Tasks"]
        )
        self.projects_radio_buttons = RadioButtonWidget(
            ["Active Projects", "Completed Projects", "All Projects"]
        )
        ...
        self.tasks_sort_combobox.addItems(
            ["TaskID", "Decription", "Deadline", "Created", "Completed", "ProjectID"]
        )
        ...
        self.projects_sort_combobox.addItems(
            ["ProjectID", "Decription", "Deadline", "Created", "Completed"]
        )
        ...
        self.tasks_radio_buttons.radio_button_group.buttonClicked.connect(
            self.populate_tasks_table
        )
        self.projects_radio_buttons.radio_button_group.buttonClicked.connect(
            self.populate_projects_table
        )

        self.tasks_sort_combobox.currentIndexChanged.connect(self.sort_tasks_table)
        self.projects_sort_combobox.currentIndexChanged.connect(
            self.sort_projects_table
        )
    ...
    def mark_project_completed(self):
        ...
        if not self.check_project_tasks_completed():
            mark_project_tasks_complete_dialog = MarkProjectTasksCompleteDialog(
                project_id
            )
